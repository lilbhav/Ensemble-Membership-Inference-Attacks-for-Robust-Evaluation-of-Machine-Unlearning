...
"""Bridge entrypoint between this repo and Third_Party_Code/MachineUnlearning.

How this file works:
1. Receives normalized CLI args from adapters/machine_unlearning_adapter.py.
2. Validates canonical split artifacts generated by scripts/prepare_splits.py.
3. Imports and calls third-party code directly from MachineUnlearning:
    - dataset/model/metrics/utils modules
    - unlearn_strategies.strategies.<method>
4. Saves model checkpoint and sidecar JSON files consumed by downstream scripts.

This file intentionally keeps third-party algorithm code untouched and focuses on
orchestration, validation, and reproducible I/O.
"""
...
def parse_args() -> argparse.Namespace:
    # These arguments form the contract between adapters/* and this bridge.
...
def main() -> None:
    ...
    # engine_repo points to Third_Party_Code/MachineUnlearning (configured in experiment.yaml).
    ...
    # Dynamically load third-party modules from MachineUnlearning.
    # This avoids copying algorithm implementations into this repository.
    ...
    # Dataset loading is delegated to third-party code so training and unlearning
    # operate on the same data pipeline as the upstream implementation.
    ...
    if args.mode == "baseline":
        # Baseline mode stays fully inside this bridge and does not call
        # unlearning strategy functions.
        ...
        # Namespace mirrors what third-party strategy functions expect.
    ...
    metrics = {
        # These metrics are produced with third-party evaluation utilities and
        # normalized into a stable schema for downstream aggregation scripts.
    ...
    # Output contract consumed by scripts/run_*.py and aggregate scripts:
    # - model checkpoint (.pt)
    # - metrics sidecar (.metrics.json)
    # - debug sidecar (.debug.json)
